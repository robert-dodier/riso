...
public class ExactlyOne_Discrete implements PiHelper
{
	...
	/** Returns a description of the sequences of distributions accepted
	  * by this helper -- namely one <tt>ExactlyOne</tt>
	  * followed by any number of <tt>Discrete</tt>.
	  */
	static
	{
		SeqTriple[] s = new SeqTriple[2];
		s[0] = new SeqTriple( "riso.distributions.ExactlyOne", 1 );
	...
	{
		ExactlyOne py = (ExactlyOne) py_in;

		// The zero'th element of each incoming pi-message is interpreted
		// as a "special" state. All the others are lumped together.
		// The output of the "exactly one" gate should be understood as the probability
		// that exactly one of the inputs is non-zero.
